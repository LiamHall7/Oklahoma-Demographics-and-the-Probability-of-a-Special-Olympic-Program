...
leaflet(all_location) %>%
  ...
  addPolygons(stroke = FALSE, smoothFactor = 0.3, fillOpacity = 1,
              fillColor = ~pal(na_percent)) %>%
  addLegend(pal = pal, values = ~ na_percent, opacity = 1.0)
...
posterior <- stan_glm(data = all_school,
                      formula = native_amer ~ median_house_income + capita_income,
                      refresh = 0)
...
sook_schools <- read_excel("raw_data/year_12_sook_school_list.xlsx") %>% 
  mutate(sook = sook_schools$District %in% all_school$sd_clean)
...
sum(if_else(all_school$sd_clean %in% sook_schools$District, 1, 0))

?unique
#   mutate(sook = map( ~ all_school$sd_clean %in% sook_schools$District))
# 
# View(all)
#the w says there might be another character, but it's optional
